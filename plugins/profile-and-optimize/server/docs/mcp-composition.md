...
[`perf-lake-contract.md`](perf-lake-contract.md) so future
...
| --- | --- | --- |
| Back-of-the-envelope performance triage | `perftune://repo/docs/performance-hints.md` or `search_runbooks` | [`inference-performance-hints`](../../skills/inference-performance-hints/SKILL.md), then the measurement skill it selects |
...
| AI-assisted tuning proposals / ledger / finalize | `ai_tuning_*` family (subverbs via `args`. An `ai_tuning_experiment submit --execute` call requires structured `i_understand_this_submits_jobs=true`. Raw ack flags in `args` are rejected) | Operator runs the resulting Slurm submissions through documented launch wrappers |
...
| Reproducibility-grade evidence bundle scaffolding | `evidence_init` (writes SOURCE.md / summary.md / commands/) | The [`evidence-bundle-init`](../../skills/evidence-bundle-init/SKILL.md) skill orchestrates the full capture |
...
| Inference perf-tuning campaign lifecycle | `perf_tune_report_campaign_init` / `cell_run` / `campaign_run` (ack-gated) / `atlas_aggregate` / `report_render` / `publish_to_lake` (external publish is ack-gated) | The [`inference-perf-tune-report`](../../skills/inference-perf-tune-report/SKILL.md) skill orchestrates the full flow |
...
| Fleet-level analysis views | `perf_tune_report_fleet_leaderboard` / `value_view` / `portability_view` / `trend_view` / `champion_select` / `tpm_summary` | The [`inference-fleet-leaderboard`](../../skills/inference-fleet-leaderboard/SKILL.md) and [`inference-value-ledger`](../../skills/inference-value-ledger/SKILL.md) skills |
| Known-good serving-config drift check | `known_good_config_record` / `known_good_config_check` (fail-closed on missing boot-blockers) | The [`inference-known-good-config`](../../skills/inference-known-good-config/SKILL.md) skill |
| Anchored metrics / profiling queries | The [`prometheus-anchored-query`](../../skills/prometheus-anchored-query/SKILL.md) and [`zymtrace-anchored-query`](../../skills/zymtrace-anchored-query/SKILL.md) skills against your metrics MCPs | Normalize lake-backed results per [`perf-lake-contract.md`](perf-lake-contract.md) |
| GitHub PRs, issues, and repo metadata | GitHub MCP or `gh` CLI per task rules | Code search tooling for cross-repo examples |


## Safety Rules

- Tools whose contract declares an acknowledgement require the matching
  `i_understand_this_*` field and mirror the underlying CLI guardrail. Current
  ack-gated tools submit jobs, change Slurm node state, or publish data to
  external storage.
...
| Observability and perf data | Prometheus / Grafana MCPs, ClickHouse MCPs | Live metrics, logs, and profiling-backend queries. Normalize results through [`perf-lake-contract.md`](perf-lake-contract.md). |
