...
keyed by the experiment-id join key (AGENTS.md "Experiment isolation and
...
in the perf-report bundle as data, not Python, per AGENTS.md.
"""
...
def build_index(campaigns_root: Path, published: set[str] | None = None) -> list[dict[str, Any]]:
    ...
    rows = [build_index_row(d, published) for d in sorted(campaigns_root.iterdir()) if _is_campaign_dir(d)]
...
def _bundle_display(value: Any) -> str:
    """Render a bundle basename without exposing an operator's absolute path."""
...
def render_index_md(rows: list[dict[str, Any]]) -> str:
    ...
    for r in rows:
        bundle_disp = _bundle_display(r.get("evidence_bundle_path"))
        lines.append(
...
# count keyed by the run-id join key (AGENTS.md "Experiment isolation and
...
def build_inventory(
    ...
    return {
        ...
        "published_local_campaigns": (
            sum(1 for r in campaigns if r.get("published_to_lake")) if published is not None else None
        ),
...
def render_inventory_md(inv: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        'campaign (AGENTS.md "Experiment isolation and traceability"). Pass `--bundle-root` '
