    ...
    for d in sorted(campaigns_root.iterdir()):
        ...
        if atlas.is_file():
            try:
                ...
                logger.debug("Skipping malformed campaign atlas %s: %s", atlas, exc)
                continue
    return rows
...
def resolve_gpu_hr(hardware: str, configs_dir: Path | None = None, override: float | None = None) -> float:
    ...
    if configs_dir is not None:
        ...
        if cost_yaml.is_file():
            try:
                data = yaml.safe_load(cost_yaml.read_text()) or {}
                if not isinstance(data, dict):
                    return GPU_HR_DEFAULT
                rates = data.get("usd_per_gpu_hour") or {}
...
def build_aa(rows: list[AtlasCell]) -> dict[str, dict[tuple[str, int], dict[str, Any]]]:
    ...
    for r in rows:
        ...
        if seen_at.get((model, key), "") <= cap:
            ...
            entry = {
                "ttft": r.ttft_avg_ms,
                "opu": r.output_tps_per_user,
                "opg": r.output_tps_per_gpu,
    ...
    for r in rows:
        ...
        if key not in best or tps > best[key]["tps_gpu"]:
            best[key] = {
                ...
                "tps_gpu": tps,
                "conc": r.concurrency,
                "tps_user": r.output_tps_per_user,
                "ttft": r.ttft_avg_ms,
                "mnbt": r.max_num_batched_tokens,
            }
...
def render_aa_md(data: dict, hw: str, gpu_hr: float) -> str:
    L = [
        f"# Artificial Analysis (AA) fleet leaderboard - {hw} TP4 NVFP4",
        "",
    ...
    for s in SHAPES:
        ranked = sorted(
            [(m, data[m][(s, 1)]) for m in data if (s, 1) in data[m]], key=lambda r: -(r[1].get("opu") or 0)
        )
        if not ranked:
            continue
        L += [
            ...
            f"### {s} (c=1)",
            "",
            "| rank | model | TTFT (ms) | output tok/s/user | tok/s/GPU |",
            "| ---: | --- | ---: | ---: | ---: |",
        ]
    ...
    L += [
        "",
        "## Answer latency (TTFO, reasoning-aware; c=1) - TTFO is an ANSWERED-SUBSET stat, always read with its coverage",
    ]
    for s in SHAPES:
        ranked = sorted(
            [(m, data[m][(s, 1)]) for m in data if (s, 1) in data[m] and data[m][(s, 1)].get("cov") is not None],
    ...
    for s in ("aa-1k", "aa-10k"):
        ranked = sorted(
            [(m, data[m][(s, 10)]) for m in data if (s, 10) in data[m]], key=lambda r: -(r[1].get("opg") or 0)
        )
        if not ranked:
            continue
        L += [
            ...
            f"### {s} (c=10)",
            "",
            "| rank | model | TTFT (ms) | tok/s/user | tok/s/GPU |",
            "| ---: | --- | ---: | ---: | ---: |",
        ]
        for i, (m, v) in enumerate(ranked, 1):
            L.append(f"| {i} | {m} | {_f(v['ttft'])} | {_f(v['opu'])} | {_f(v['opg'])} |")
    ...
    def dol(value: float | None) -> str:
        return f"${value:.2f}" if isinstance(value, (int, float)) else "(DNF)"

    for m, c1, c10, c10k in crows:
        L.append(f"| {m} | {dol(c1)} | {dol(c10)} | {dol(c10k)} |")
    L += [
        "",
        "Batching (c=10) is ~7x cheaper than single-prompt (c=1). Companion: "
...
def render_throughput_md(allrows: list[dict], hw: str, gpu_hr: float) -> str:
    def table(rs: list[dict], header: str) -> list[str]:
        ...
        out = [
            header,
            "",
            "| rank | model | quant | TP | peak tok/s/GPU | @conc | tok/s/user@peak | TTFT@peak (ms) | $/1M out |",
            "| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
        for i, r in enumerate(rs, 1):
            c = _cost(r["tps_gpu"], gpu_hr)
            out.append(
                f"| {i} | {r['model']} | {r['quant']} | {r['tp']} | {_f(r['tps_gpu'])} | {r['conc']} | "
                f"{_f(r['tps_user'])} | {_f(r['ttft'])} | {f'${c:.2f}' if c else 'NA'} |"
            )
        return out
    ...
    L = [
        f"# Throughput fleet leaderboard (peak tok/s/GPU) - {hw} NVFP4",
        "",
...
def render_pareto_md(M: dict, hw: str, gpu_hr: float, quality: dict | None = None) -> str:
    ...
    L = [
        f"# Fleet model-selection guide (perf Pareto frontier) - {hw} TP4 NVFP4",
        "",
    ...
    L += [
        "## Perf-efficient frontier (non-dominated - choose from these)",
        "",
    ...
    for n in sorted(frontier, key=lambda x: -M[x]["tps_gpu"]):
        e = M[n]
        L.append(
            f"| {n} | {_f(e['lat_user'])} | {_f(e['lat_ttft'])} | {_f(e['tps_gpu'])} | "
            f"${_cost(e['tps_gpu'], gpu_hr):.2f} |"
        )
    L += ["", "## Perf-dominated (beaten on ALL three axes)", ""]
    if dominated:
        L += [
            "| model | decode tok/s/user | TTFT (ms) | peak tok/s/GPU | dominated by |",
            "| --- | ---: | ---: | ---: | --- |",
        ]
        for n in sorted(dominated, key=lambda x: -M[x]["tps_gpu"]):
            ...
            L.append(
                f"| {n} | {_f(e['lat_user'])} | {_f(e['lat_ttft'])} | {_f(e['tps_gpu'])} | {', '.join(dom) or '-'} |"
            )
    else:
        L.append("_None - every measured model is on the perf frontier._")
    L += [
        "",
        "## Model quality (lm-eval serving evals, where measured)",
    ...
    if measured:
        ...
        for m in sorted(measured):
            cells = " | ".join(
                (_f(measured[m].get(n), 3) if measured[m].get(n) is not None else "-") for n in metric_names
            )
            L.append(f"| {m} | {cells} |")
    else:
    ...
    L += [
        "",
        "## Critical caveat: PERF-only Pareto, NOT model quality",
...
def write_leaderboards(
    rows: list[AtlasCell], out_dir: Path, hw: str = "GB300", gpu_hr: float = GPU_HR_DEFAULT
) -> dict[str, int | str]:
