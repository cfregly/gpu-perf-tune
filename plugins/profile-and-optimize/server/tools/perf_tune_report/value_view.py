...
def build_value_view(registry: dict, campaigns_root: Path) -> dict[str, Any]:
    ...
    for f in registry.get("findings", []):
        ...
        for cid in cids:
            ...
            if (
                f.get("finding_baseline_check", True)
                and not st["baseline_named"]
            ...
            flags.extend(
                _prov.provenance_match_problems(
                    f.get("source_refs"),
                    _read_campaign_provenance(cdir),
                    label=cid,
                )
            )
...
def render_markdown(view: dict, *, title: str = "Value ledger", gpu_hr: float = _fleet.GPU_HR_DEFAULT) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "Summary: " + " | ".join(f"{lc}={counts.get(lc, 0)}" for lc, _ in LIFECYCLE_GROUPS),
        "",
    ...
    for lc, heading in LIFECYCLE_GROUPS:
        ...
        for f in fs:
            ...
            tiers = ", ".join(sorted({c.get("verdict_tier", "?") for c in live["campaigns"] if c.get("found")})) or "-"
...
def _finding_economics(f: dict, gpu_hr: float) -> dict[str, Any]:
    ...
    dollars_saved = dollars_base - dollars_peak if (dollars_peak is not None and dollars_base is not None) else None
    gpu_hours_saved = (
        (1.0 / 3600.0) * 1e6 * (1.0 / base_tps - 1.0 / tps_peak)
        if isinstance(tps_peak, (int, float)) and isinstance(base_tps, (int, float)) and tps_peak and base_tps
        else None
    )
...
def render_frontier(view: dict, *, gpu_hr: float = _fleet.GPU_HR_DEFAULT) -> str:
    ...
    missing = [f.get("id", "?") for f in view["findings"] if not str(f.get("next_lever", "") or "").strip()]
    lines: list[str] = [
    ...
    if not active:
        lines.append(
            "> WARNING: no ACTIVE next levers queued -- the frontier looks exhausted. "
            "Verify every finding's `next_lever` (the ratchet forbids a stall)."
        )
        lines.append("")
    else:
        lines.append(
            "| # | gain | $/1M out (peak) | $ saved/1M | GPU-hrs saved/1M | Next lever | From finding | Lifecycle |"
        )
        lines.append("|---|---|---|---|---|---|---|---|")
    ...
    if missing:
        lines.append(
            f"> RATCHET FLAG: {len(missing)} finding(s) missing `next_lever`: "
            + ", ".join(missing)
            + " -- name the next lever."
        )
        lines.append("")
    if exhausted:
        lines.append(
            f"_Banked / frontier-exhausted ({len(exhausted)}): " + ", ".join(r["id"] for r in exhausted) + "._"
        )
        lines.append("")
    return "\n".join(lines)
...
def render_report(
    view: dict, *, title: str = "Inference perf wins -- value prop", gpu_hr: float = _fleet.GPU_HR_DEFAULT
) -> str:
    ...
    counts = _count_by_lifecycle(view)
    summary = ", ".join(f"{_GROUP_LABEL[lc].split(' -- ')[0]} {counts.get(lc, 0)}" for lc, _ in LIFECYCLE_GROUPS)
    lines: list[str] = [
        f"# {title}",
    ...
    for lc, _ in LIFECYCLE_GROUPS:
        ...
        for f in fs:
            ...
            lines.append(f"- **{f.get('title', '')}** -- {f.get('win', '')}  _({'; '.join(meta)})_")
        lines.append("")
    lines.append(render_frontier(view, gpu_hr=gpu_hr))
