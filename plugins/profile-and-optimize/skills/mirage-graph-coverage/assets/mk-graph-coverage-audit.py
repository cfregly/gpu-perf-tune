...
def audit_rank(gendir, rank, critical):
    ...
    in_refs, out_refs, in_tt, _out_tt, ntasks = parse_consumed(tgj)
    ...
    crit = {
        c: {
            "declared": c in declared,
            "in_refs": in_refs.get(c, 0),
            "consumers": sorted(x for x in in_tt.get(c, set()) if x is not None),
            "out_refs": out_refs.get(c, 0),
        }
        for c in critical
    }
    return {
...
def main():
    ...
    if a.ranks:
        ...
        ranks = []
        for path in glob.glob(os.path.join(a.gendir, "task_graph_*.json")):
            match = re.search(r"task_graph_(\d+)\.json", path)
    ...
    for r in ranks:
        ...
        if res is None:
            ...
            fail = True
            continue
        mt = datetime.datetime.fromtimestamp(res["mtime"], datetime.UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        for c in critical:
            ...
            if ci["declared"] and ci["in_refs"] == 0:
                ...
                status = "not-declared"
            print(
                f"  [{status}] {c}: declared={ci['declared']} input_refs={ci['in_refs']} "
